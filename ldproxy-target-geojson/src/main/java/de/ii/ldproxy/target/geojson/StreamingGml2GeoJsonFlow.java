 ...
 * @author zahnen
 */
public class StreamingGml2GeoJsonFlow implements StreamingGmlTransformerFlow.GmlTransformerFlow {

    public static Flow<ByteString, ByteString, NotUsed> transformer(final QName featureType, final WfsProxyFeatureTypeMapping featureTypeMapping) {
        return StreamingGmlTransformerFlow.transformer(featureType, featureTypeMapping, new StreamingGml2GeoJsonFlow(true));
    }

    private static final LocalizedLogger LOGGER = XSFLogger.getLogger(StreamingGml2GeoJsonFlow.class);
    ...
    @Override
    public void onGeometryStart(TargetMapping mapping, WfsProxyFeatureTypeAnalyzer.GML_GEOMETRY_TYPE type, Integer dimension) throws IOException {
        if (Objects.nonNull(mapping)) {
            stopBuffering();
