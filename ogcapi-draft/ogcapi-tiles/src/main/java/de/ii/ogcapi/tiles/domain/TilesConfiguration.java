...
/**
 * @en Example of the specifications in the configuration file from the API for
...
/**
 * @en For the vineyard objects (aggregation of adjacent objects up to zoom level 9):
 * @de Für die Weinlagen-Objekte (Aggregation von aneinander angrenzenden Objekten bis Zoomstufe 9):
 * @example <code>
 * ```yaml
 * - buildingBlock: TILES
 *   rules:
...
/**
 * @en Example of a simple configuration (no seeding at startup, rebuilding the cache every hour):
 * @de Beispiel für eine einfache Konfiguration (kein Seeding beim Start, Neuaufbau des Cache zu jeder Stunde):
 * @example <code>
 * ```yaml
 * - buildingBlock: TILES
 *   tileProvider:
...
/**
 * @en Example of using multiple threading for seeding:
 * @de Beispiel für die Verwendung von mehreren Threads für das Seeding:
 * @example <code>
 * ```yaml
 * - buildingBlock: TILES
 *   tileProvider:
...
/**
 * @en For this, at least 4 threads must be configured for background processes in the global configuration, for example:
 * @de Hierfür müssen in der globalen Konfiguration mindestens 4 Threads für Hintergrundprozesse konfiguriert sein, zum Beispiel:
 * @example <code>
 * ```yaml
 * backgroundTasks:
 *   maxThreads: 4
 * ```
 * </code>
 * @en These seedings make multiple background processes possible in the first place. So, even without
 ...
 * @de Durch diese Setzungen werden mehrere Hintergrundprozesse überhaupt erst ermöglicht.
...
/**
 * @en Example configuration (from the API [Low resolution satellite imagery (OpenMapTiles preview)](https://demo.ldproxy.net/openmaptiles)):
 * @de Beispielkonfiguration (aus der API[Satellitenbilder in niedriger Auflösung (OpenMapTiles-Preview)](https://demo.ldproxy.net/openmaptiles)):
 * @example <code>
 * ```yaml
 * - buildingBlock: TILES
 *   enabled: true
...
/**
 * @en Ein Beispiel für eine TileServer-GL-Konfiguration mit dem Style "topographic", der z.B.
 * als Datenquelle die Vector Tiles der API verwenden kann:
 * @de Ein Beispiel für eine TileServer-GL-Konfiguration mit dem Style "topographic", der z.B.
 * als Datenquelle die Vector Tiles der API verwenden kann:
 * @example <code>
 * ```json
 * {
...
public interface TilesConfiguration extends ExtensionConfiguration, PropertyTransformations, CachingConfiguration {
    ...
    /**
     * @en Specifies the data source for the tiles, see [Tile provider objects](#tile-provider).
     * @de Spezifiziert die Datenquelle für die Kacheln, siehe [Tile-Provider-Objekte](#tile-provider).
    ...
    /**
     * @en Controls which formats are supported for the tileset resources.
     ...
     * @de Steuert, welche Formate für die Tileset-Ressourcen unterstützt werden sollen. Zur Verfügung
    ...
    /**
     * @en `FILES` stores each tile as a file in the file system. `MBTILES` stores the tiles in an
     ...
     * @de `FILES` speichert jede Kachel als Datei im Dateisystem. `MBTILES` speichert die Kacheln
    ...
    /**
     * @en Selection of the map client to be used in the HTML output.
     ...
     * @de Auswahl des zu verwendenden Map-Clients in der HTML-Ausgabe. Der Standard ist MapLibre
    ...
    /**
     * @en A style in the style repository to be used in maps with tiles by default.
     ...
     * @de Ein Style im Style-Repository, der standardmäßig in Karten mit den Tiles verwendet
    ...
    /**
     * @en If `true` is selected, the `minzoom` and `maxzoom`
     ...
     * @de Bei `true` werden aus dem in `style` angegebenen Style die `minzoom`- und
    ...
    /**
     * @en *Deprecated* See [Tile-Provider Features](#tile-provider-features).
     * @de *Deprecated* Siehe [Tile-Provider Features](#tile-provider-features).
     * @default `[ "MVT" ]`
     */
    @Deprecated
