...
public class TilesDataHydrator implements OgcApiDataHydratorExtension {
    ...
    public OgcApiDataV2 getHydratedData(OgcApiDataV2 apiData) {
        ...
        Map<String, TilesConfiguration> collectionConfigs = apiData.getCollections()
                                                                   .entrySet()
                                                                   .stream()
                                                                   .map(entry -> {
                                                                       final FeatureTypeConfigurationOgcApi collectionData = entry.getValue();
                                                                       TilesConfiguration config = collectionData.getExtension(TilesConfiguration.class).orElse(null);
        ...
        if (Objects.nonNull(apiConfig)) {
            ...
            builder.extensions(new ImmutableList.Builder<ExtensionConfiguration>()
                                       // do not touch any other extension
                                       .addAll(apiData.getExtensions()
                                                      .stream()
        ...
        builder.collections(apiData.getCollections()
                                   .entrySet()
                                   .stream()
                                   .map(entry -> {
                                       final String collectionId = entry.getKey();
    ...
    private TilesConfiguration process(OgcApiDataV2 apiData, TilesConfiguration config) {
        if (Objects.nonNull(config.getTileProvider())
            ...
            try {
                ...
                Map<String, MinMax> zoomLevels = ImmutableMap.of("WebMercatorQuad", new ImmutableMinMax.Builder()
                ...
                config = new ImmutableTilesConfiguration.Builder()
                        .from(config)
                        .tileProvider(ImmutableTileProviderMbtiles.builder()
                                                                  .from(tileProvider)
