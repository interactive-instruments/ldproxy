...
public class EndpointSortables extends EndpointSubCollection /* implements ConformanceClass */ {
  ...
  @Inject
  public EndpointSortables(ExtensionRegistry extensionRegistry,
  ...
  protected ApiEndpointDefinition computeDefinition(OgcApiDataV2 apiData) {
    ...
    if (!optCollectionIdParam.isPresent()) {
      ...
      for (String collectionId : collectionIds) {
        // TODO summary and description needs to be updated to describe the Sortables resource, not the Queryables resource
        final List<OgcApiQueryParameter> queryParameters = getQueryParameters(extensionRegistry, apiData, path, collectionId);
        final String operationSummary = "retrieve the sortables of the feature collection '" + collectionId + "'";
        Optional<String> operationDescription = Optional.of("The sortables resources identifies the properties that can be" +
                                                                "referenced in the 'sortby' parameter to order the features of the collection in the response to a query." +
                                                                "The response is returned as a JSON Schema document that describes a single JSON object where each property is a sortable.\n\n"+
                                                                "Note that the sortables schema does not specify a schema of any object that can be retrieved from the API.\n\n"+
                                                                "JSON Schema is used for the sortables to have a consistent approach for describing schema information and JSON Schema " +
  ...
  public Response getSortables(@Auth Optional<User> optionalUser,
                               @Context OgcApi api,
