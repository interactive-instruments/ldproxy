...
public abstract class FeatureTransformationContextCityJson implements FeatureTransformationContext {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FeatureTransformationContextCityJson.class);
    ...
    @Value.Default
    public boolean getTextSequences() { return false; }

    @Value.Default
    public CityJsonConfiguration.Version getVersion() { return CityJsonConfiguration.Version.V11; }
    ...
    @Value.Default
    public boolean getPrettify() { return false; }
    ...
    public final void startAttributes() throws IOException {
        Objects.requireNonNull(getState()).getAttributesBuffer().ifPresent(val -> getState().setAttributesBufferBuilding(val));
    ...
    public final void startAddress() throws IOException {
        Objects.requireNonNull(getState()).getAddressBuffer().ifPresent(val -> getState().setAddressBufferBuilding(val));
        TokenBuffer buffer = createJsonBuffer();
    ...
    public final void startGeometry() throws IOException {
        Objects.requireNonNull(getState()).getGeometryBuffer().ifPresent(val -> getState().setGeometryBufferBuilding(val));
        TokenBuffer buffer = createJsonBuffer();
    ...
    public void writeAttributeValue(String value, SchemaBase.Type type) throws IOException {
        ...
        switch (type) {
            case BOOLEAN:
                ...
                buffer.writeBoolean(value.equalsIgnoreCase("t") || value.equalsIgnoreCase("true") || value.equals("1"));
                break;
            case INTEGER:
                try {
            ...
            case FLOAT:
                try {
                    ...
                    LOGGER.error("Error writing floating point value in CityJSON, writing string. Found: {}", value);
                    buffer.writeString(value);
                }
                break;
            default:
    ...
    public Optional<Integer> processOrdinate(String value) {
        ...
        if (getState().getCurrentVertex().size()==3 && state.getCurrentVertices().isPresent()) {
            ...
            for (int i=0; i<3; i++) {
                transformed[i] = Math.round((state.getCurrentVertex().get(i) - state.getCurrentTranslate().get(i)) / state.getCurrentScale().get(i));
            }
    ...
    @Value.Modifiable
    public static abstract class StateCityJson extends State {

        // Transitions:
        // OUTSIDE -> IN_BUILDING
        // IN_BUILDING -> IN_ADDRESS | IN_GEOMETRY | IN_GEOMETRY_WITH_SURFACES | IN_GEOMETRY_IGNORE | IN_SURFACES_IGNORE | OUTSIDE
        // IN_ADDRESS -> previous
        ...
        public enum Section { OUTSIDE, IN_BUILDING, IN_ADDRESS, IN_GEOMETRY, IN_GEOMETRY_IGNORE, IN_GEOMETRY_WITH_SURFACES, WAITING_FOR_SURFACES, IN_SURFACES, IN_SURFACE_GEOMETRY, IN_SURFACES_IGNORE }

        @Value.Default
        public Section inSection() { return Section.OUTSIDE; }

        @Value.Default
        public Section previousSection() { return Section.OUTSIDE; }

        @Value.Default
        public boolean inBuildingPart() { return false; }

        @Value.Derived
        @Value.Auxiliary
        public boolean atTopLevel() { return inSection()==Section.IN_BUILDING || inSection()==Section.WAITING_FOR_SURFACES; }

        @Value.Derived
        @Value.Auxiliary
        public boolean inActiveGeometry() { return inSection()==Section.IN_GEOMETRY || inSection()==Section.IN_GEOMETRY_WITH_SURFACES; }
