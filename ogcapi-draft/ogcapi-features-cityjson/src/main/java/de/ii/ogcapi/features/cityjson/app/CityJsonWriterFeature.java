...
public class CityJsonWriterFeature implements CityJsonWriter {
    ...
    public void onFeatureStart(EncodingAwareContextCityJson context, Consumer<EncodingAwareContextCityJson> next) throws IOException {
        ...
        if (context.encoding().getTextSequences()) {
            context.encoding().getJson()
                   .writeStartObject();
            context.encoding().getJson()
                   .writeStringField(TYPE, CityJsonWriter.CITY_JSON_FEATURE);
            context.encoding().getJson()
                   .writeFieldName(CITY_OBJECTS);
            context.encoding().getJson()
                   .writeStartObject();
        }
    ...
    public void onFeatureEnd(EncodingAwareContextCityJson context,
        ...
        context.encoding().getJson()
               .writeFieldName(getId(context));
        context.encoding().getJson()
               .writeStartObject();
        // the only top-level features are buildings
        context.encoding().getJson()
               .writeStringField(TYPE, CityJsonWriter.BUILDING);

        if (!context.getState()
                    .getCurrentChildren()
                    .isEmpty()) {
            context.encoding().getJson()
                   .writeFieldName(CityJsonWriter.CHILDREN);
            context.encoding().getJson()
                   .writeStartArray();
            for (String child : context.getState()
                                       .getCurrentChildren()) {
                context.encoding().getJson()
                       .writeString(child);
            }
    ...
    public void onFeatureEndEnd(EncodingAwareContextCityJson context, Consumer<EncodingAwareContextCityJson> next) throws IOException {
        ...
        if (context.encoding().getTextSequences()) {
            ...
            context.encoding().getJson()
                .writeStringField(ID, getId(context));
            context.encoding().getJson()
                   .writeEndObject();
            context.encoding().getJson()
                   .flush();
    ...
    public void onObjectEnd(EncodingAwareContextCityJson context,
        ...
        if (context.schema().isPresent()
            ...
            context.encoding().getJson()
                   .writeFieldName(buildingPartId);
            context.encoding().getJson()
                   .writeStartObject();
            context.encoding().getJson()
                   .writeStringField(TYPE, CityJsonWriter.BUILDING_PART);

            context.getState()
                   .addCurrentChildren(buildingPartId);
            context.encoding().getJson()
                   .writeFieldName(CityJsonWriter.PARENTS);
            context.encoding().getJson()
                   .writeStartArray();
            context.encoding().getJson()
                   .writeString(getId(context, true));
            context.encoding().getJson()
                   .writeEndArray();
