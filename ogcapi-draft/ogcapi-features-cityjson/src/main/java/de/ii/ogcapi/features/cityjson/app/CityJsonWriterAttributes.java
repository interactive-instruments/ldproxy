...
public class CityJsonWriterAttributes implements CityJsonWriter {
    ...
    @Override
    public void onFeatureEnd(EncodingAwareContextCityJson context, Consumer<EncodingAwareContextCityJson> next) throws IOException {
    ...
    public void onObjectStart(EncodingAwareContextCityJson context,
        ...
        if (context.schema().isPresent() && context.schema().get().isObject()) {
            ...
            if (context.getState().inBuildingPart()
                ...
                if (schema.isArray()) {
                    context.getState().getAttributesBuffer().get()
                           .writeStartObject();
                } else {
                    context.getState().getAttributesBuffer().get()
                           .writeObjectFieldStart(schema.getName());
                }
            }
        }
            ...
            if (schema.isArray()) {
                ...
                if (!name.equals("gml_id")) {
                    context.getState().getAttributesBuffer().get()
                           .writeFieldName(name);
                    context.encoding()
                           .writeAttributeValue(value, schema.getType());
                }
            }
        }
