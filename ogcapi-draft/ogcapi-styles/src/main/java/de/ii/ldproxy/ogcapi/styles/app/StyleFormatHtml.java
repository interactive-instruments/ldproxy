...
public class StyleFormatHtml implements StyleFormatExtension {
    ...
    public Response getStyleResponse(String styleId, File stylesheet, List<Link> links, OgcApi api, ApiRequestContext requestContext) throws IOException {
        ...
        if (layerControl) {
            ...
            mbStyle.getLayers()
                   .stream()
                   .filter(styleLayer -> styleLayer.getSource().isPresent() && vectorSources.contains(styleLayer.getSource().get()) && styleLayer.getSourceLayer().isPresent())
                   .forEach(styleLayer -> layerMap.put(api.getData().getCollections().get(styleLayer.getSourceLayer().get()).getLabel(),styleLayer.getId()));
        }
