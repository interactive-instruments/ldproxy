...
public abstract class StyleMetadata extends OgcResourceMetadata {
  ...
  public static final String SCHEMA_REF = "#/components/schemas/StyleMetadata";
  ...
  public StyleMetadata replaceParameters(String serviceUrl) {
    ...
    boolean templated =
        this.getStylesheets().stream()
                .map(styleSheet -> styleSheet.getLink().orElse(null))
                .filter(Objects::nonNull)
                .anyMatch(
                    link ->
                        Objects.requireNonNullElse(link.getTemplated(), false)
                            && link.getHref().matches("^.*\\{serviceUrl\\}.*$"))
            || this.getLayers().stream()
    ...
    return ImmutableStyleMetadata.builder()
                ...
                .map(
                    styleSheet ->
                        ImmutableStylesheetMetadata.builder()
                            .from(styleSheet)
                            .link(
                                !(styleSheet.getLink().isPresent())
                                    ? Optional.empty()
                                    : Objects.requireNonNullElse(
                                            styleSheet.getLink().get().getTemplated(), false)
                                        ? Optional.of(
        ...
        .layers(
            this.getLayers().stream()
                .map(
                    layer ->
                        ImmutableStyleLayer.builder()
                            .from(layer)
                            .sampleData(
                                !(layer.getSampleData().isPresent())
                                    ? Optional.empty()
                                    : Objects.requireNonNullElse(
                                            layer.getSampleData().get().getTemplated(), false)
                                        ? Optional.of(
                                            new ImmutableLink.Builder()
                                                .from(layer.getSampleData().get())
                                                .href(
                                                    layer
                                                        .getSampleData()
                                                        .get()
            ...
            this.getLinks().stream()
                .map(
                    link ->
                        new ImmutableLink.Builder()
                            .from(link)
                            .href(link.getHref().replace("{serviceUrl}", serviceUrl))
                            .templated(null)
                            .build())
                .collect(ImmutableList.toImmutableList()))
        .build();
