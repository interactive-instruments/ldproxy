 ...
 * @langEn The *Features Custom Extensions* module can be enabled for any API provided over ldproxy
 *     with a feature provider.
 *     <p>It adds support for the HTTP method POST on the features resource. The difference to
 *     calling with GET is that the query parameters are passed as content in the request. This may
 *     be desired for two reasons:
 *     <p>* URLs are limited in length in HTTP implementations. Extensive filter expressions in GET
 *     calls are often too long. Using POST gets around this limitation. * When using POST, query
 *     parameters are transmitted in encrypted form when using HTTPS and are not logged in request
 ...
 *     <p>* `intersects` (resource "features"): if the parameter is specified, the features are
 ...
 * @langDe Das Modul *Features Custom Extensions* kann für jede über ldproxy bereitgestellte API mit
 *     einem Feature-Provider aktiviert werden.
 *     <p>Es ergänzt die Unterstützung der HTTP-Methode POST auf der Features-Ressource. Der
 *     Unterschied zum Aufruf mit GET ist, dass die Query-Parameter als Content im Aufruf übergeben
 *     werden. Dies kann aus zwei Gründen gewünscht sein:
 *     <p>* URLs sind in HTTP-Implementierungen in der Länge beschränkt. Umfangreiche
 ...
 *     <p>* `intersects` (Ressource "Features"): Ist der Parameter angegeben, werden die Features
