...
|`validateTypes` |enum |`IGNORE` |Steuert ob die Spezifikationen der Objektarten daraufhin geprüft werden, ob sie zur Datenquelle passen (nur für SQL). `IGNORE` heißt keine Prüfung, bei `WARN` werden Warnungen geloggt, bei `ERROR` werden Fehler geloggt und der Provider wird nicht gestartet.
