...
|`tags` |array |`null` |Ordnet der API die aufgelisteten Tags zu. Die Tags müssen jeweils String ohne Leerzeichen sein. Die Tags werden im API-Katalog angezeigt und können über den Query-Parameter `tags` zur Filterung der in der API-Katalog-Antwort zurückgelieferten APIs verwendet werden, z.B. `tags=INSPIRE`.
