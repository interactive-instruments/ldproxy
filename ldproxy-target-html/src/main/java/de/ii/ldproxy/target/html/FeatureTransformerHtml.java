...
import static de.ii.ldproxy.output.html.MicrodataGeometryMapping.MICRODATA_GEOMETRY_TYPE.POLYGON;
...
public class FeatureTransformerHtml implements FeatureTransformer {
    ...
    public FeatureTransformerHtml(OutputStreamWriter outputStreamWriter, boolean isFeatureCollection, boolean isAddress, List<String> groupings, boolean isGrouped, String query, int[] range, FeatureCollectionView featureTypeDataset, CrsTransformer crsTransformer, SparqlAdapter sparqlAdapter, CodelistStore codelistStore, ViewRenderer mustacheRenderer) {
    ...
    @Override
    public void onStart(OptionalInt numberReturned, OptionalInt numberMatched) throws Exception {

        LOGGER.debug("START");

        if (isFeatureCollection && numberReturned.isPresent()) {
            int returned = numberReturned.getAsInt();
            int matched = numberMatched.orElse(-1);
    ...
    @Override
    public void onAttribute(TargetMapping mapping, String value) throws Exception {
        writeField((MicrodataPropertyMapping) mapping, value, true);
    }

    @Override
    public void onPropertyStart(TargetMapping mapping) throws Exception {
    ...
    public void onPropertyEnd() throws Exception {
        writeField((MicrodataPropertyMapping) currentMapping, currentValue.toString(), false);
    }
    ...
    @Override
    public void onGeometryStart(TargetMapping mapping, GML_GEOMETRY_TYPE type, Integer dimension) throws Exception {
