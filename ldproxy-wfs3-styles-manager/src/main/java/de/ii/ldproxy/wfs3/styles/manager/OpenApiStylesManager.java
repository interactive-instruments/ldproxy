...
public class OpenApiStylesManager implements Wfs3OpenApiExtension {
    ...
    public OpenAPI process(OpenAPI openAPI, Wfs3ServiceData serviceData) {
        ...
        if (serviceData != null &&
            isExtensionEnabled(serviceData, StylesConfiguration.class) &&
            getExtensionConfiguration(serviceData, StylesConfiguration.class).isPresent()) {
            ...
            if (stylesExtension.getManagerEnabled()) {
                ...
                if (stylesExtension.getValidationEnabled()) {
                    ...
                    validate.setSchema(new StringSchema()._enum(validateEnum)._default("no"));
                    openAPI.getComponents()
                            .addParameters("validate", validate);
                }

                Content contentStylesheet =  pathItemStyle.getGet().getResponses().get("200").getContent();
