...
public interface JsonSchemaCache {
  ...
  default JsonSchemaDocument getSchema(
      OgcApiDataV2 apiData, String collectionId, Optional<String> schemaUri, VERSION version, FeaturesCoreProviders providers, ConcurrentMap<Integer, ConcurrentMap<String, ConcurrentMap<VERSION, JsonSchemaDocument>>> cache) {
    ...
    if (!cache.get(apiHashCode).get(collectionId).containsKey(version)) {
      ...
      cache.get(apiHashCode)
          .get(collectionId)
          .put(version, deriveSchema(featureType, collectionData, schemaUri, version));
    }

    return cache.get(apiHashCode).get(collectionId).get(version);
  }

  JsonSchemaDocument deriveSchema(FeatureSchema schema, FeatureTypeConfigurationOgcApi collectionData, Optional<String> schemaUri, VERSION version);
