...
public class GeoJsonWriterCrs implements GeoJsonWriter {
  ...
  public void onStart(
    ...
    if (context.encoding().isFeatureCollection()
      ...
      writeCrs(
          ...
          context.encoding().getDefaultCrs());
    }
  ...
  private void writeCrs(
      JsonGenerator json, Optional<CrsTransformer> crsTransformer, EpsgCrs defaultCrs)
      throws IOException {
    if (crsTransformer.isPresent()
        && !Objects.equals(crsTransformer.get().getTargetCrs(), defaultCrs)) {
      json.writeStringField("crs", crsTransformer.get().getTargetCrs().toUriString());
    }
  }
}
