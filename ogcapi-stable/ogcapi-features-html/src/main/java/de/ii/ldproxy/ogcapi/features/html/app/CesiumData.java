...
import de.ii.ldproxy.ogcapi.features.html.domain.Geometry.Coordinate;
...
public class CesiumData {
  ...
  private void addPolygon(ImmutableList.Builder<String> builder, String name, Geometry.Polygon polygon, Double minHeight) {
    ...
    if (polygon.getCoordinates().size()==1) {
      builder.add("viewer.entities.add({" +
          "name:\"" + name + "\"," +
          (is3d
              ? "polygon:{hierarchy:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(polygon.getCoordinates().get(0).getCoordinates(), minHeight) + "]),perPositionHeight:true,"
              : "polygon:{hierarchy:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(polygon.getCoordinates().get(0).getCoordinates()) + "]),perPositionHeight:true,") +
      ...
      builder.add("viewer.entities.add({" +
          "name:\"" + name + "\"," +
          (is3d
              ? "polygon:{hierarchy:{positions:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(polygon.getCoordinates().get(0).getCoordinates(), minHeight) + "])," +
              "holes:[" +
              IntStream.range(1, polygon.getCoordinates().size())
                  .mapToObj(n -> "{positions:Cartesian3.fromDegreesArrayHeights(["+getCoordinatesString(polygon.getCoordinates().get(n).getCoordinates(), minHeight)+"])}")
                  .collect(Collectors.joining(",")) +
              "]},perPositionHeight:true,"
              : "polygon:{hierarchy:{positions:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(polygon.getCoordinates().get(0).getCoordinates()) + "])," +
                  "holes:[" +
                  IntStream.range(1, polygon.getCoordinates().size())
                      .mapToObj(n -> "{positions:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(polygon.getCoordinates().get(n).getCoordinates()) + "])}")
                      .collect(Collectors.joining(",")) +
                  "]},perPositionHeight:true,") +
          "material:Color.BLUE.withAlpha(0.5)," +
          "outline:true,outlineColor:Color.BLUE" +
  ...
  private void addLineString(ImmutableList.Builder<String> builder, String name, Geometry.LineString line, Double minHeight) {
    builder.add("viewer.entities.add({" +
        "name:\"" + name + "\"," +
        (line.is3d()
            ? "polyline:{positions:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(line.getCoordinates(), minHeight) + "]),perPositionHeight:true,"
            : "polyline:{positions:Cartesian3.fromDegreesArrayHeights([" + getCoordinatesString(line.getCoordinates()) + "]),perPositionHeight:true,") +
        // "outline:true,outlineColor:Color.BLUE," +
        "width:1," +
        "material:Color.BLUE" +
        "}});");
  ...
  private void addPoint(ImmutableList.Builder<String> builder, String name, Geometry.Point point, Double minHeight) {
    builder.add("viewer.entities.add({" +
        "name:\"" + name + "\"," +
        "position:Cartesian3.fromDegrees(" + getCoordinatesString(point.getCoordinates(), minHeight) + ")," +
