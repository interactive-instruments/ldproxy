...
/**
 * @title Collection
 * @en Every collection corresponds to a feature type defined in the feature provider
 * (only *Feature Collections* are currently supported).
 * @de Jedes Collection-Objekt beschreibt eine Objektart aus einem Feature Provider
...
public interface FeatureTypeConfigurationOgcApi extends FeatureTypeConfiguration, ExtendableConfiguration, Buildable<FeatureTypeConfigurationOgcApi> {
    ...
    /**
     * @en The *Feature* resource defines a unique URI for every feature, but this URI is
     ...
     * @de Über die Feature-Ressource hat jedes Feature zwar eine feste URI, die für Links
    ...
    /**
     * @en Array of additional link objects, required keys are `href` (the URI), `label` and `rel` (the relation).
     * @de Erlaubt es, zusätzliche Links bei jeder Objektart zu ergänzen. Der Wert ist ein Array von
