...
public interface OgcApiPathParameter extends ParameterExtension {
    default boolean isExplodeInOpenApi(OgcApiDataV2 apiData) { return false; }
    ...
    default boolean isApplicable(OgcApiDataV2 apiData, String definitionPath, Optional<String> collectionId) {
        return collectionId.map(s -> isApplicable(apiData, definitionPath, s)).orElseGet(() -> isApplicable(apiData, definitionPath));
    }
