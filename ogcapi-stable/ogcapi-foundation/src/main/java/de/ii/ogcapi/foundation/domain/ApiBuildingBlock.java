 ...
 * @langEn # Overview
 * <p>
 * The OGC API functionality is split up into modules based on the OGC API standards. The
 ...
 * @langDe # Übersicht
 * <p>
 * Die API-Funktionalität ist in Module, die sich an den OGC API Standards orientieren,
 ...
 * ### Option `enabled`
 * <p>
 * Every module can be enabled or disabled in the configuration using `enabled`. The default value differs between modules.
 * <p>
 * ### Resource paths
 * <p>
 ...
 * <p>
 * Modules related to feature encoding ([Core](features_core.md), [GeoJSON](geo_json.md), [HTML](features_html.md), [Vector Tiles](tiles.md)) support transforming feature properties for all or only for specific encodings.
 * <p>
 * Transformations do not affect data sources, they are applied on-the-fly as part of the encoding.
 * <p>
 * Filter expressions do not take transformations into account, they have to be based on the source values. That means queryable properties (see `queryables` in [Features Core](features_core.md)) should not use transformations in most cases. The exception to the rule is the HTML encoding, where readability might be more important than filter support.
 * <p>
 * See [Transformations](../../providers/details/transformations.md) for supported transformations.
 ...
 * ### Option `enabled`
 *</code>
 * Jedes API-Modul hat eine Konfigurationsoption `enabled`, die steuert, ob das Modul in der jeweiligen API aktiviert ist. Einige Module sind standardmäßig aktiviert, andere deaktiviert.
 * <p>
 * ### Pfadangaben
 * <p>
 ...
 * ### Transformationen
 * <p>
 * In den API-Modulen, die Features verarbeiten ([Core](features_core.md), [GeoJSON](geo_json.md), [JSON-FG](json_fg.md), [HTML](features_html.md), [Tiles](tiles.md) mit dem Features-Tile-Provider), können die Feature-Eigenschaften über Transformationen an die Anforderungen der Ausgabe angepasst werden.
 * <p>
