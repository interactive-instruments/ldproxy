...
public abstract class Tile {
  ...
  /**
   * @return the ids of the collections included as layers in the tile, empty list means all
   *     collections
   */
  ...
  /**
   * @return {@code true}, if the tile is not stored in the persistent cache, it is only stored for
   *     a few minutes to support generating multi-layer tiles from multiple single-layer tiles
   */
  ...
  public String getCollectionId() {
    return !isDatasetTile() && getCollectionIds().size() == 1 ? getCollectionIds().get(0) : null;
  }
  ...
  public Path getRelativePath() {
    ...
    if (this.getTemporary()) {
      tilePath = Paths.get(String.format("%s.%s", UUID.randomUUID(), extension));
    } else {
      tilePath =
          Paths.get(
              String.valueOf(getTileLevel()),
              String.valueOf(getTileRow()),
              String.format("%d.%s", getTileCol(), extension));
    }
  ...
  /**
   * Verify that the zoom level is in the valid range for the tile matrix set. Verify that the row
   * number is in the valid range for the tile matrix. Verify that the column number is in the valid
   * range for the tile matrix. Otherwise throw an IllegalStateException exception.
   */
  @Value.Derived
  @Value.Auxiliary
  public void check() {
    if (getTileLevel() > getTileMatrixSet().getMaxLevel()
        || getTileLevel() < getTileMatrixSet().getMinLevel())
      throw new IllegalStateException(
          MessageFormat.format(
              "Tile is not valid in tiling scheme {0}, zoom level {1} is outside of the range {2}..{3}.",
              getTileMatrixSet().getId(),
              getTileLevel(),
              getTileLevel() < getTileMatrixSet().getMinLevel(),
              getTileMatrixSet().getMaxLevel()));
    if (!getTileMatrixSet().validateRow(getTileLevel(), getTileRow()))
      throw new IllegalStateException(
          MessageFormat.format(
              "Tile is not valid in tiling scheme {0}, row {1} is outside of the range for zoom level {2}.",
              getTileMatrixSet().getId(), getTileRow(), getTileLevel()));
    if (!getTileMatrixSet().validateCol(getTileLevel(), getTileCol()))
      throw new IllegalStateException(
          MessageFormat.format(
              "Tile is not valid in tiling scheme {0}, column {1} is outside of the range for zoom level {2}.",
              getTileMatrixSet().getId(), getTileCol(), getTileLevel()));
  }

  /**
   * Creates an affine transformation for converting geometries in native CRS of the tiling scheme
   * to tile coordinates.
   *
   ...
   *
   * @param crsTransformerFactory the coordinate reference system transformation object to transform
   *     coordinates
   * @return the transform
   ...
   * @throws CrsTransformationException an error occurred when transforming the coordinates
   */
  public BoundingBox getBoundingBox(EpsgCrs crs, CrsTransformerFactory crsTransformerFactory)
